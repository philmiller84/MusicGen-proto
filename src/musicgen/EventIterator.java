...
public class EventIterator {
	...
	/**
	 * Takes the Events in this iterator and adds them in order to another
	 * EventIterator
	 * 
	 * What I want is for these Dispense methods to be in UpperLevelEventLine
	 *  and to take another EventLine in as a parameter, not an Iterator but I
	 * can't, since the EventIterators that go with an EventLine are created in main
	 * An EventLine doesn't seem to own
	 * its iterators. It creates a new EventIterator instead,
	 * 
	 * I'd suggest putting some of the EventIterator methods in EventLine and
	 * not exposing the iterators to main
	 * 
	 * @param otherIterator
	 * @param positionType
	 */
	public void dispenseEvents(EventIterator otherIterator,
			PositionType positionType) {

		First(); // set currentEvent

		while (currentEvent != null) {
			otherIterator.CreateEventAtPosition(currentEvent, positionType);
	...
	/**
	 * If the Events in this iterator are UpperLevelEvents which implement Generator,
	 *  this takes all the
	 * events in all the Generators and puts them somewhere.
	 * 
	 * 
	 * 
	 * @param otherIterator
	 * @param positionType
	 */
	public void dispenseEventsInIterators(EventIterator otherIterator,
			PositionType positionType) {
		// all of this casting is occurring because EventLine does not have
		// access to its EventIterator. The EventIterator is created in the
		// scope of main, so I can't implement these methods inside
		// ofUpperLevelEvent, since UpperLevelEvent can't create its own iterator
		// Consequently, I need to use EventIterator, which
		// casts references to the UpperLevelEvents in an UpperLevelEventLine to
		// type Event and then they must be cast back in order to do this
		First(); // set currentEvent
		
			

			while (currentEvent != null) {
				if (currentEvent instanceof UpperLevelEvent) {
					// since the methods in EventIterator return references to
					// Event, even if the object pointed to is of type
					// UpperLevelEvent, change the reference type
					UpperLevelEvent generatingEvent;
					generatingEvent = ((UpperLevelEvent) currentEvent);
					generatingEvent.generatedIter.dispenseEvents(otherIterator,
							positionType);
